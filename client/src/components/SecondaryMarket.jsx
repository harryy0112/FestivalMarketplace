...
class SecondaryMarket extends Component {
  ...
  updateTickets = async () => {
    try {
      ...
      const saleTickets = await nftInstance.methods
        .getTicketsForSale()
        .call({ from: initiator });
      const renderData = await Promise.all(
        saleTickets.map(async (ticketId) => {
          const { purchasePrice, sellingPrice, forSale } =
          ...
          if (forSale) {
            return (
              <tr key={ticketId}>
                ...
                <td class="center">
                  {web3.utils.fromWei(sellingPrice, "ether")}
                </td>
  ...
  onPurchaseTicket = async (ticketId, sellingPrice, initiator) => {
    try {
      ...
      renderNotification(
        "success",
        "Success",
        "Ticket purchased for the festival successfully!"
      );
    } catch (err) {
  ...
  updateFestivals = async () => {
    try {
      ...
      renderNotification(
        "danger",
        "Error",
        "Error while updating the fetivals"
      );
    ...
    return (
      <div class="container center">
        <div class="row">
          <div class="container ">
            <div class="container ">
              <h5 style={{ padding: "30px 0px 0px 10px" }}>
              ...
              <table id="requests" class="responsive-table striped">
                <thead>
                  <tr>
                    ...
                    <th key="ticketId" class="center">
                      Ticket Id
                    </th>
                    <th key="cost" class="center">
                      Cost(in FEST)
                    </th>
